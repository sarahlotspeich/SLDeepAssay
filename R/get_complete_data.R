...
get_complete_data_big = function(Wstar, Zstar) {
  # Save useful constants 
  M = length(Wstar) ## number of replicate wells 
  MP = sum(Wstar) ## number p24-positive replicate wells 
  n = nrow(Zstar) ## number of DVLs detected
...
# get_complete_data_big = function(n, m, Wstar_seq, Zstar_seq) {
#   # Create 2 ^ n x n matrices of DVL combinations
#   z = data.matrix(expand.grid(
#     replicate(n = n, 
#               expr = c(0, 1), 
#               simplify = F)
#     )
#   )
#   
#   # Reorder to put the row of all 0s first
#   order_z = order(rowSums(z) != 0)
#   z = z[order_z, ]
#   
#   # Create matrix of complete data for sequenced wells
#   # Augment complete data with observed data 
#   ## For sequenced wells 
#   cd_seq_long = cbind(j = rep(x = 1:m, each = nrow(z)),
#                       z[rep(x = 1:nrow(z), times = m), ], 
#                       Zstar_seq[rep(x = 1:m, each = nrow(z)), , drop = F], 
#                       wstar = rep(x = Wstar_seq, each = nrow(z)))
# }